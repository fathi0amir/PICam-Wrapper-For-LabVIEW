...
extern "C" __declspec(dllexport) void GetFrame(PicamHandle camera, unsigned short* image);
extern "C" __declspec(dllexport) void GetFrame(PicamHandle camera, unsigned short* image)
{
...
extern "C" __declspec(dllexport) void CloseCamera(PicamHandle camera, char* closeCameraMessage);
extern "C" __declspec(dllexport) void CloseCamera(PicamHandle camera, char* closeCameraMessage)
{
    Picam_CloseCamera(camera);
    std::string tempString = "DemoPIMAX3 Closed";
    strcpy_s(closeCameraMessage, tempString.length() + 1, tempString.c_str());
}

extern "C" __declspec(dllexport) void SetCameraParameters(PicamHandle camera, long long* accumulations);
extern "C" __declspec(dllexport) void SetCameraParameters(PicamHandle camera, long long* accumulations)
{
    Picam_SetParameterLargeIntegerValue(camera, PicamParameter_Accumulations, (pi64s)*accumulations);
...
extern "C" __declspec(dllexport) void GetCameraParameters(PicamHandle camera, long long* accumulations);
extern "C" __declspec(dllexport) void GetCameraParameters(PicamHandle camera, long long* accumulations)
{
    Picam_GetParameterLargeIntegerValue(camera, PicamParameter_Accumulations, accumulations);
}